// Arquivo: /api/orcamentos.js (Versão 1.3.11 - Estável)
  ...
  switch (method) {
    case 'GET':
      try {
        ...
        // Rota para buscar o HISTÓRICO de um cliente (ATUALIZADA com total_pago)
        else if (cliente_id) {
          const { rows } = await dbPool.query(
            `SELECT 
               o.id, o.codigo_orcamento, o.data_criacao, o.valor_total, o.status,
               COALESCE((SELECT SUM(valor_pago) FROM pagamentos WHERE orcamento_id = o.id), 0) as total_pago 
             FROM orcamentos o 
